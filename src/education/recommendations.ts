...
import { EffectType, DE_ESSER_PARAM } from '@/types/effects'
...
export function computeRecommendations(
  ...
  const hasSat       = hasType(effects, EffectType.Saturation)
  const hasDeEsser   = hasType(effects, EffectType.DeEsser)
  const hasExpander  = hasType(effects, EffectType.Expander)

  const deEsser   = effects.find((e) => e.type === EffectType.DeEsser)
  ...
  // ── Spectral → suggest De-Esser when presence band is high ───────────────

  if (bands && isPlaying) {
    const { presence, upperMids } = bands
    if (presence > 0.45 && presence > upperMids * 1.3 && !hasDeEsser) {
      out.push({
        id: 'spectral:sibilance-suggest-deesser',
        severity: 'suggestion',
        effectType: EffectType.DeEsser,
        ro: {
          beginner: 'Frecvențele înalte (4–8 kHz) sunt puternice, posibil sibilante. Un De-Esser controlează automat aceste frecvențe fără a le tăia permanent.',
          advanced: 'Energie presence band >0.45 — sibilanță potențială. De-esser-ul (sidechain HPF ~6 kHz) aplicat selective pe vârfuri de sibilanță, spre deosebire de EQ static care taie permanent.',
        },
        en: {
          beginner: 'High frequencies (4–8 kHz) are strong, possibly sibilant. A De-Esser controls these frequencies automatically without permanently cutting them.',
          advanced: 'Presence band energy >0.45 — potential sibilance. De-esser (sidechain HPF ~6 kHz) applies gain reduction selectively on sibilant peaks, unlike static EQ which cuts permanently.',
        },
      })
    }
  }

  // ── Saturation without EQ after ───────────────────────────────────────────

  if (hasSat && !hasEq) {
    const satIdx = effects.findIndex((e) => e.type === EffectType.Saturation)
    const eqAfterSat = effects.slice(satIdx + 1).some((e) => e.type === EffectType.ParametricEq)
    if (!eqAfterSat) {
      out.push({
        id: 'chain:sat-no-eq-after',
        severity: 'suggestion',
        effectType: EffectType.ParametricEq,
        ro: {
          beginner: 'Saturarea adaugă armonice noi care pot îngreuna sunetul. Un EQ după Saturation poate tăia frecvențele nedorite generate de distorsie.',
          advanced: 'Saturation fără EQ corectiv post-procesare: armonicele înalte (>8 kHz) generate de waveshaper pot fi aspre. EQ post-sat cu HiShelf –2..–4 dB sau bell cut la frecvența de "frizz" îmbunătățește tonul.',
        },
        en: {
          beginner: 'Saturation adds new harmonics that can make the sound heavy. An EQ after Saturation can remove unwanted frequencies generated by distortion.',
          advanced: 'Saturation without corrective post-EQ: high harmonics (>8 kHz) generated by the waveshaper can be harsh. Post-sat EQ with HiShelf –2..–4 dB or bell cut at the "frizz" frequency improves tone.',
        },
      })
    }
  }

  // ── De-Esser with LISTEN mode on ─────────────────────────────────────────

  if (deEsser && (deEsser.params[DE_ESSER_PARAM.LISTEN] ?? 0) >= 0.5) {
    out.push({
      id: 'chain:deesser-listen-on',
      severity: 'warning',
      ro: {
        beginner: 'De-Esser-ul tău are LISTEN activat — auzi sidechain-ul, nu semnalul procesat. Dezactivează LISTEN înainte de export.',
        advanced: 'De-Esser LISTEN=1: semnalul de ieșire este sidechain-ul HPF, nu audio-ul procesat. Utilitar de tuning — dezactivează obligatoriu înainte de mixdown.',
      },
      en: {
        beginner: 'Your De-Esser has LISTEN enabled — you hear the sidechain signal, not the processed output. Disable LISTEN before exporting.',
        advanced: 'De-Esser LISTEN=1: output is the HPF sidechain, not the processed audio. Tuning utility — must disable before mixdown.',
      },
    })
  }

  // ── Noisy signal: suggest Expander or Gate ────────────────────────────────

  if (isPlaying && masterRms < 0.035 && masterPeak > 0.05 && !hasGate && !hasExpander) {
    const crestDb = toDB(masterPeak) - toDB(masterRms)
    if (crestDb > 18) {
      out.push({
        id: 'signal:noisy-suggest-expander',
        severity: 'suggestion',
        effectType: EffectType.Expander,
        ro: {
          beginner: 'Semnalul pare să aibă mult zgomot de fundal (nivel general scăzut dar cu vârfuri). Un Expander sau Gate ar putea reduce zgomotul între sunetele utile.',
          advanced: `Crest factor ridicat (${(toDB(masterPeak) - toDB(masterRms)).toFixed(1)} dB) și RMS scăzut → noise floor semnificativ relativ la semnal. Expander downward cu threshold −50..−40 dB, ratio 2:1–4:1 reduce etajul de zgomot lin.`,
        },
        en: {
          beginner: 'The signal appears to have significant background noise (low average level but with peaks). An Expander or Gate could reduce noise between useful sounds.',
          advanced: `High crest factor (${(toDB(masterPeak) - toDB(masterRms)).toFixed(1)} dB) and low RMS → significant noise floor relative to signal. Downward expander with threshold −50..−40 dB, ratio 2:1–4:1 smoothly reduces noise floor.`,
        },
      })
    }
  }
