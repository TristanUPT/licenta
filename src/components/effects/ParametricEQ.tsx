    ...
    <EffectCard instance={instance}>
      ...
      <div className={`mb-3 w-full rounded-lg border px-4 py-3 transition ${
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-[100px] flex-1 border-l border-zinc-800 pl-4">
            <p className="text-[10px] leading-relaxed text-zinc-500">
              {ro ? 'Elimină frecvențele sub-bass sub pragul de tăiere.' : 'Removes sub-bass content below the cutoff.'}{' '}
              {lcEnabled
                ? (ro
                    ? `Activ la ${(instance.params[EQ_LOW_CUT_PARAM.FREQ] ?? 80).toFixed(0)} Hz, ${lcSlope === 1 ? '12' : '24'} dB/oct.`
                    : `Active at ${(instance.params[EQ_LOW_CUT_PARAM.FREQ] ?? 80).toFixed(0)} Hz, ${lcSlope === 1 ? '12' : '24'} dB/oct.`)
                : (ro ? 'Dezactivat.' : 'Disabled.')}
            </p>
          </div>
        </div>
      </div>

      {/* Bands — horizontal scroll so they never compress below readable minimum */}
        ...
        <div className="flex gap-3" style={{ minWidth: 'max-content' }}>
          {bands.map((bandIdx) => {
            ...
            return (
              <div
                key={bandIdx}
                className={`w-[200px] shrink-0 rounded-lg border p-3 transition ${
                  enabled
              ...
              >
                <div className="mb-2 flex items-center justify-between gap-1">
                  <button
                    ...
                    className={`shrink-0 rounded-md px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider transition ${
                      enabled
                        ? 'bg-emerald-600/20 text-emerald-300 hover:bg-emerald-600/30'
                        : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700'
                    }`}
                  >
                    {ro ? `Bandă ${bandIdx + 1}` : `Band ${bandIdx + 1}`}
                  </button>
                  <select
                    value={Math.round(typeValue)}
                    onChange={(e) => setParam(instance.id, typeId, Number(e.target.value))}
                    className="min-w-0 flex-1 rounded-md border border-zinc-800 bg-zinc-900 px-1 py-0.5 text-[10px] text-zinc-300 focus:border-purple-500 focus:outline-none"
                    aria-label={`${ro ? 'Bandă' : 'Band'} ${bandIdx + 1} — ${typeLabel}`}
                  >
