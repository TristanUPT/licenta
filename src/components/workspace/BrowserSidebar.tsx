...
export function BrowserSidebar() {
  ...
  async function loadFile(file: File) {
    ...
    try {
      ...
      const msg = err instanceof UnsupportedAudioFormatError
        ? (language === 'ro' ? `Format nesuportat: ${file.name}` : `Unsupported: ${file.name}`)
  ...
  function loadPreset(preset: Preset) {
    ...
    if (getStatus().status !== 'running') {
      setPresetError(language === 'ro' ? 'Pornește engine-ul mai întâi.' : 'Start the engine first.')
      return
    }
    try {
      clearEffects()
  ...
  return (
    <aside className="flex h-full w-48 shrink-0 flex-col border-r border-zinc-800 bg-zinc-950">
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
        ...
        {/* Archive / Presets */}
        <Section title={language === 'ro' ? 'Arhivă' : 'Archive'}>
          {presetError && (
        ...
        {/* Sample Library */}
        <Section title={language === 'ro' ? 'Sample Library' : 'Sample Library'}>
          <div className="px-3 pb-1.5">
            <input
              type="text"
              placeholder={language === 'ro' ? 'Caută...' : 'Search samples...'}
              value={searchQ}
      ...
      <div className="shrink-0 border-t border-zinc-800 p-3">
        ...
        >
          ...
          {language === 'ro' ? 'Importă fișier audio' : 'Import Audio File'}
        </button>
        <input
          ref={inputRef}
          type="file"
